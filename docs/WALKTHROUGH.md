...
| File | Changes |
|------|---------|
| [`api/views.py`](file:///Users/ayush/Desktop/code/flybeta-project/backend/api/views.py) | **1.** Replaced all instances of `user = get_dev_user()` with `user = request.user` across `LessonViewSet.complete` and `CapstoneSubmissionViewSet.create`.<br>**2.** Changed permission classes from `AllowAny` to `IsAuthenticated` to secure these endpoints.<br>**3.** Fixed XP math to increment `profile.total_xp` (used by the ranking system) alongside `profile.xp`.<br>**4.** Added `profile.compute_rank()` calls to immediately recalculate rank progression when XP is awarded.<br>**5.** Changed `complete` to return `UserProfileSerializer` (rich payload) instead of `UserStatsSerializer`. |
